...
class SOM(object):
    ...
    def init_map(self):
        ...
        if getattr(self, 'initmethod') == 'random':
            ...
            mn = np.tile(np.min(getattr(self, 'data'), axis=0), (getattr(self, 'nnodes'), 1)) * (1 - .0)
            mx = np.tile(np.max(getattr(self, 'data'), axis=0), (getattr(self, 'nnodes'), 1)) * (1 + .0)
    ...
    def hit_map(self, data=None):
        ...
        plt.scatter(coord[:, 1], msz[0] - .5 - coord[:, 0], s=area.flatten(), alpha=.9, c='None', marker='o',
        ...
        if data != None:
            ...
            plt.scatter(coord_d[:, 1] + .5, msz[0] - .5 - coord_d[:, 0], s=area, alpha=0.9, c='None', marker='o',
                        cmap='jet', linewidths=3, edgecolor='r')
            plt.scatter(coord_d[:, 1] + .5, msz[0] - .5 - coord_d[:, 0], s=area, alpha=0.2, c='b', marker='o',
                        cmap='jet', linewidths=3, edgecolor='r')
            print ('hi')
    ...
    def view_U_matrix(self, distance2=4, row_normalized='Yes', show_data='Yes', contooor='Yes', blob='Yes', save='Yes',
                      save_dir=''):
        import scipy
        ...
        if show_data == 'Yes':
            plt.scatter(coord[:, 1], coord[:, 0], s=2, alpha=1., c='Gray', marker='o', cmap='jet', linewidths=3,
                        edgecolor='Gray')
            plt.axis('off')
        ...
        if which_dim == 'all':
            ...
            for i in range(data.shape[1]):
                ...
                for j in range(data.shape[0]):
                    sc = plt.scatter(coords[j, 1], self.mapsize[0] - 1 - coords[j, 0], c=data[j, which_dim[i]],
                                     vmax=mx[j], vmin=mn[j], s=90, marker='.', edgecolor='None', cmap=colormap, alpha=1)
...
#         pca = RandomizedPCA(n_components=2)  # Randomized PCA is scalable
        ...
        data = (data - me)
        codebook = np.tile(me, (nnodes, 1))
        pca = PCA(svd_solver='randomized',n_components=1)
